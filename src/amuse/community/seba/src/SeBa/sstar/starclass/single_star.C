...
{ single_star* element = 0;
  ...
  //if (type >= 3){
  //	  read_in_csv_table(beta_array, "Beta_stars.csv");
 // 	  read_in_csv_table(timestamp_array, "Timestamps_stars.csv");
  //	  read_in_csv_table(k_array, "LogK2_stars.csv");
  //} else {
  //    read_in_csv_table(beta_array_SSO, "Beta_SSOs.csv");
  //    read_in_csv_table(timestamp_array_SSO, "Timestamps_SSOs.csv");
  //    read_in_csv_table(k_array_SSO, "LogK2_SSOs.csv");}
...
single_star::single_star(single_star & rv) : star(rv) {
  ...
  beta_interpolated = rv.beta_interpolated;
  k_interpolated = rv.k_interpolated;
  rotation_period = rv.rotation_period;
  for (int i=0; i<19; i++){
  	 for(int j = 0; j <25; j++){
       beta_array[i][j] = rv.beta_array[i][j];
       timestamp_array[i][j] = rv.timestamp_array[i][j];
       k_array[i][j] = rv.k_array[i][j];  
     }     
  }
...
//  }
  ...
  update();
  if (this->get_element_type() >= 3){
  	  this->read_in_csv_table(beta_array, "Beta_stars.csv");
      this->read_in_csv_table(timestamp_array, "Timestamps_stars.csv");
      this->read_in_csv_table(k_array, "LogK2_stars.csv");
      this->determine_beta(relative_mass, t_cur);
  } else {
  	  this->read_in_csv_table(beta_array_SSO, "Beta_SSOs.csv");
      this->read_in_csv_table(timestamp_array_SSO, "Timestamps_SSOs.csv");
      this->read_in_csv_table(k_array_SSO, "LogK2_SSOs.csv");
      this->determine_beta(relative_mass, t_cur);
  }
...
void  single_star::read_in_csv_table(real beta_array[19][25], char* filename){
   char buffer[1024] ;
   char *record,*line;
   int i=0,j=0;
...
void single_star::determine_beta(const real mass, const real time){
	//cout<<"before k_interpolated: "<<k_interpolated<<", beta interpolated: "<<beta_interpolated<<endl;
	// convert time to yrs from Myrs
	real time_yrs = pow(10,6) * time;
	
	const int size_mbins = 19;
	const int size_tbins = 25;
	real m_bins [size_mbins] = {0.6, 0.8, 0.85, 0.92, 1., 1.2, 1.4, 1.6,
					    	1.8, 2., 2.5, 3., 4., 5., 6., 7., 8., 9., 10.};
	
	// check whether we are in relevant parameter space
	real max_time; // !we might not need this!
	...
	max_time =  *max_element(max_time_for_a_given_mass, max_time_for_a_given_mass + size_mbins); // !we might not need this!
	

	real max_mass = *max_element(m_bins, m_bins + size_mbins);
	real min_mass = *min_element(m_bins, m_bins + size_mbins);

	if (mass >= min_mass && mass <= max_mass && time_yrs <= max_time){ 	//Is it an correct mass range?
		//First, thing first, determine which bin the mass belongs to.
		// i.e determine m_lower and m_upper
		...
		int im_upper;
		int im_lower;
		real delta_lower = -max_mass_difference ; // initialisation of difference. actual value doesn't matter as long as
		real delta_upper = max_mass_difference ;  // it's large enough
		...
		if (time_yrs > max_time_for_a_given_mass[im_lower] || time_yrs > max_time_for_a_given_mass[im_upper]){
			...
			for(int i=0;i<size_tbins;i++){
				delta_temp_m_lower = timestamp_array[im_lower][i] - time_yrs;
				if (delta_temp_m_lower > 0) {
					if (delta_temp_m_lower < delta_t_upper) {
						t_lu = timestamp_array[im_lower][i];
						it_lu = i;
					...
					if (time_yrs == max_time_for_a_given_mass[im_lower]){
						t_lu = timestamp_array[im_lower][i];
						t_ll = timestamp_array[im_lower][i-1];
						it_lu = i;
						it_ll = i-1;
					}else if (delta_temp_m_lower > delta_t_lower) {
						t_ll = timestamp_array[im_lower][i];
						it_ll = i;
			...
			for(int i=0;i<size_tbins;i++){
				delta_temp_m_upper = timestamp_array[im_upper][i] - time_yrs;
				if (delta_temp_m_upper > 0) {
					if (delta_temp_m_upper < delta_t_upper) {
						t_uu = timestamp_array[im_upper][i];
						it_uu = i;
				    ...
				    if (time_yrs == max_time_for_a_given_mass[im_upper]){
						...
						break;
							...
							t_ul = timestamp_array[im_upper][i];
							it_ul = i;
			...
			real beta_m_lower_at_t = beta_array[im_lower][it_ll] + (time_yrs - t_ll) * (beta_array[im_lower][it_lu] - beta_array[im_lower][it_ll]) / (t_lu - t_ll);
			real beta_m_upper_at_t = beta_array[im_upper][it_ul] + (time_yrs - t_ul) * (beta_array[im_upper][it_uu] - beta_array[im_upper][it_ul]) / (t_uu - t_ul);
			...
			real k_m_lower_at_t = k_array[im_lower][it_ll] + (time_yrs - t_ll) * (k_array[im_lower][it_lu] - k_array[im_lower][it_ll]) / (t_lu - t_ll);
			real k_m_upper_at_t = k_array[im_upper][it_ul] + (time_yrs - t_ul) * (k_array[im_upper][it_uu] - k_array[im_upper][it_ul]) / (t_uu - t_ul);
		...
		//cout<<"k_interpolated: "<<k_interpolated<<", beta interpolated: "<<beta_interpolated<<endl;
