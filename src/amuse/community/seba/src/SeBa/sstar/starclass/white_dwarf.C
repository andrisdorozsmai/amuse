...
//    (GN+SilT Mar 2010) was cnsts.parameters(kanonical_neutron_star_mass)
      ...
      accreted_mass = 0;
      //cout<<"masses wd1:"<<relative_mass<<", "<<get_total_mass()<<","<<beta_interpolated<<","<<k_interpolated<<endl;
...
//      last_update_age = next_update_age;
    ...
    relative_age = 0.;//1 + nucleair_evolution_time();
      ...
      post_constructor();
      //cout<<"masses wd2:"<<relative_mass<<", "<<get_total_mass()<<","<<beta_interpolated<<","<<k_interpolated<<endl;
...
//    real t_nuc = nucleair_evolution_time();
  ...
  //      if (t_nuc>=relative_age) {
      ...
      determine_beta(get_mzams(), current_time); 
      //if (identity == 0){
